...
#ifndef HASHLIB_HASHTAB_H_INCLUDED
#define HASHLIB_HASHTAB_H_INCLUDED 1

#include "hash/standard.h"

/* PRIVATE TYPES AND DEFINITIONS */

struct Hashlib_privatescope_hitem
{
  ub1          *key;      /* key that is hashed */
  ub4           keyl;     /* length of key */
  void         *stuff;    /* stuff stored in this hitem */
  ub4           hval;     /* hash value */
  struct Hashlib_privatescope_hitem *next;     /* next hitem in list */
};
typedef  struct Hashlib_privatescope_hitem  Hashlib_privatescope_hitem;


struct Hashlib_privatescope_htab
{
  struct Hashlib_privatescope_hitem **table;   /* hash table, array of size 2^logsize */
  ...
  struct Hashlib_privatescope_hitem  *ipos;    /* current item in the array */
  struct Hashlib_privatescope_reroot *space;   /* space for the hitems */
  ub4            bcount;  /* # hitems useable in current block */
};
typedef  struct Hashlib_privatescope_htab  Hashlib_privatescope_htab;
 ...
 */
#define Hashlib_privatescope_hcount(t) ((t)->count)
#define Hashlib_privatescope_hkey(t)   ((t)->ipos->key)
#define Hashlib_privatescope_hkeyl(t)  ((t)->ipos->keyl)
#define Hashlib_privatescope_hstuff(t) ((t)->ipos->stuff)
 ...
 */
word  Hashlib_privatescope_hfind( Hashlib_privatescope_htab *t, ub1 *key, ub4 keyl );
 ...
 */
word  Hashlib_privatescope_hadd( Hashlib_privatescope_htab *t, ub1 *key, ub4 keyl, void *stuff );
 ...
 */
word  Hashlib_privatescope_hdel( Hashlib_privatescope_htab *t );


/* Hashlib_privatescope_hfirst - move position to the first item in the table
  ARGUMENTS:
    t    - the hash table
  RETURNS:
    FALSE if there is no current item (meaning the table is empty)
  NOTE:
 */
word Hashlib_privatescope_hfirst( Hashlib_privatescope_htab *t );
...
/* hnext - move position to the next item in the table
  ...
  NOTE:
    To see every item in the table, do
      if (Hashlib_privatescope_hfirst(t)) do
      {
        key   = hkey(t);
        stuff = hstuff(t);
      }
...
#define Hashlib_privatescope_hnext(t) \
  ((!(t)->ipos) ? FALSE :  \
   ((t)->ipos=(t)->ipos->next) ? TRUE : Hashlib_privatescope_hnbucket(t))
 ...
 */
word Hashlib_privatescope_hnbucket( Hashlib_privatescope_htab *t );
 ...
 */
void Hashlib_privatescope_hstat( Hashlib_privatescope_htab *t );

#endif   /* HASHLIB_HASHTAB_H_INCLUDED */
